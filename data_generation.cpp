    ...
    config::DUMMY_DATA get_evened()
    {
        ...
        for (auto& chunk : dataset)
        {
            ...
            (
                ...
                config::CHUNK_SIZE,
                [counter_max = static_cast<std::size_t>(std::round(1. / Job::CHANCE_OF_HEAVY_JOB_APPEARING)), counter = 0ULL]
                () mutable
                {
                    ++counter;
                    counter %= counter_max;
                    if (counter == 0ULL) return Job{ std::make_unique<HeavyTask>() };
                    else return Job{ std::make_unique<LightTask>() };
                }
            );
        }
        return dataset;
