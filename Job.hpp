...
public:
    ...
    static std::unique_ptr<Task> generate_task() noexcept 
    {
        ...
        static std::bernoulli_distribution is_heavy_job{ CHANCE_OF_HEAVY_JOB_APPEARING };

        if (!is_heavy_job(rng)) return std::make_unique<LightTask>();
        else return std::make_unique<HeavyTask>();
    }
