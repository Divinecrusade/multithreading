...
void StatisticChunk::save_as_csv(std::span<StatisticChunk const> stats,
  ...
  for (auto const& chunk : stats) {
    ...
    for (std::size_t i{0ULL}; i < config::SLAVES_COUNT; ++i) {
      ...
      csv << std::format("{};{};{};", chunk.timing_per_thread[i], cur_idle,
                         chunk.number_of_heavy_jobs_per_thread[i]);
      total_idle += cur_idle;
    }
    csv << std::format("{};{};{}", chunk.total_timing,
                       std::accumulate(
                           chunk.number_of_heavy_jobs_per_thread.begin(),
                           chunk.number_of_heavy_jobs_per_thread.end(), 0ULL),
                       total_idle)
        << std::endl;
  }
}
